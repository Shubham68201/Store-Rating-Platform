...
import { addStore, getUsers } from '../../features/adminSlice';
...
const AddStore = () => {
    ...
    const { isLoading, error, users } = useSelector((state) => state.admin);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!validate()) return;
        const result = await dispatch(addStore(form));
        if (!result.error) {
    ...
    return (
        <div className="max-w-lg mx-auto p-6 page-enter">
            <h1 className="text-2xl font-bold gradient-text mb-6">Add New Store</h1>
            <div className="glass-card p-8">
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-1">Owner (optional)</label>
                    ...
                    <div className="flex gap-3 pt-2">
                        <button type="submit" disabled={isLoading}
                            ...
                            {isLoading ? <span className="loading loading-spinner loading-sm"></span> : 'Add Store'}
                        </button>
